...
<div class="container">
	...
	methods: {
		...
		seleccionarRespuestaEmpareja(pregunta, indexResp) {
			...
			if (this.preguntaSeleccionadaJuegoEmparejamiento) {
				...
				for (
					...
					if (
						this.preguntasCuestionarioRespuestas[p]
							.respuestaEstudiante
					) {
		...
		calcularPuntuacion() {
			...
			if (this.totalTime <= 1.0) {
				...
				this.puntosObtenidos =
					100 * this.aciertos.length * this.totalTime
				this.puntos = Number(this.puntos) + Number(this.puntosObtenidos) //se acumulan los puntos al numero original de puntos
			...
			var numeroSubmoduloAprobadosPorCurso = this
				.submodulosAprobadosPorCurso.length
			...
			if (this.apruebaEvaluacion == 1 && !this.submoduloAprobado) {
				...
				numeroSubmoduloAprobadosPorCurso = this
					.submodulosAprobadosPorCurso.length
			...
			this.porcentajeAvanceSubmodulos =
				(numeroSubmoduloAprobadosPorCurso /
					this.numeroSubmodulosCurso) *
				100
		...
		guardarIntentoEvaluacion() {
			...
			axios({
				...
				.then(response => {
					this.usuario.ultimoIntento =
						response.data.intentoEvaluacionCreado
				})
				.catch(err => {
					alert(
						'Error: no se puede guardar el avance en este momento: ' +
							err
					)
				})
		},
	...
	computed: {
		totalTimeProgress() {
			let porcentaje =
				(this.totalTime / this.tiempoMaximoPorPregunta) * 100
			return porcentaje
		},
		subeDeNivel_Comp() {
