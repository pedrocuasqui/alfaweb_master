...
module.exports = {
	...
	fn: async function(inputs, exits) {
		...
		var modulo = await ModuloLibro.findOne({ id: inputs.moduloId }).intercept(
			err => {
		...
		var modulos = await ModuloLibro.find({ curso: modulo.curso }).populate(
			"submodulos"
		);
		var curso = await Curso.findOne({ id: modulo.curso }).intercept(err => {
			sails.log("ERROR doble populate: " + err);
		});
