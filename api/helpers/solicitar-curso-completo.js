...
module.exports = {
	...
	description:
		"devuelve el objeto curso con sus respectivos modulos y submodulos",
	...
	fn: async function(inputs) {
		...
		try {
			...
			curso = await Curso.findOne({ id: inputs.objetoId });
			...
			if (curso) {
				...
				modulosCurso = await ModuloLibro.find({ curso: curso.id }).populate(
					"submodulos"
				);
		...
		try {
			...
			modulo = await ModuloLibro.findOne({ id: inputs.objetoId }).intercept(
				err => {
					sails.log("ERROR MODULO NO ENCONTRADO", err);
				}
			);
		...
		try {
			...
			submodulo = await SubmoduloLibro.findOne({ id: inputs.objetoId });
			// if (Object.keys(submodulo).lenght > 0) {
			if (submodulo) {
				//buscar el modulo al que pertenece el submodulo
				modulo = await ModuloLibro.findOne({ id: submodulo.modulo });
