 ...
 * Example usage:
 * ```
 *     return res.unauthorizes();
 *     // -or-
 *     return res.unauthorizes(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function unauthorizes(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.unauthorizes()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.unauthorizes()` called with an Error:', optionalData);
